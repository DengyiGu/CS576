...
def _anomaly_scores(features: list[_WindowFeatures]) -> list[float]:
    ...
    also in loudness, spectral centroid/rolloff, and noisiness.  Combining
    these signals (ported from the Dengyi branch) makes music-only or no-dialog
    ads easier to separate from normal static content.
    """
    if not features:
        return []
