...
def _asr_spans_in_range(t0: float, t1: float, speech_spans: list[SpeechSpan]) -> list[SpeechSpan]:
    return [
        span
        for span in speech_spans
        if span.t1 >= t0
...
def _semantic_structure_scores_for_range(
    t0: float,
    t1: float,
    speech_spans: list[SpeechSpan],
    *,
    ...
    return intro_score, outro_score
...
def _has_opening_title_signal(
    ...
    title_term_hit = any(term in ocr_text for term in _OPENING_TITLE_TERMS)
    visual_title_ratio = _graphics_ratio(windows, indices, t0, t1)
    semantic_intro_score, _ = _semantic_structure_scores_for_range(t0, t1, speech_spans)
    ...
    return (
        title_term_hit
        or title_card_text_shape
        or semantic_supported_title
    )
...
def _build_non_ad_segments(
    ...
    intro_end = _edge_intro_end_time(windows, run_indices, speech_spans, intro_used)
